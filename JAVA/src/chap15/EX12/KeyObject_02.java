 ...
 * 		*블락동기화의 경우 Key를 임의의 객체로 지정할 수 있음으로 위와 같은 문제를 피할 수 있다.
 * 		Key가 같은 메소드에 접근한 경우에는 하나씩 처리되며, Key가 다른 메소드와 동시에 처리된다.
 */
...
class MyData {																			// 공유객체 : 여러 스레드가 공유 객체에 접근
	...
	void cde() {
		synchronized (new Object()) {													// 익명 객체를 Key로 블락 동기화
			for (int i = 0; i < 3; i++) {
